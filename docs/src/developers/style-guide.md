...
!!! info
    The style guide is always a work in progress, and not all Fibonacci code
    follows the rules. When modifying Fibonacci, please fix the style violations
...
Fibonacci uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/Fibonacci.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the Fibonacci directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to Fibonacci have
